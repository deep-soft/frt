...
//  FRT <wc>					Show files that comply with <wc> + final count (find)

//  FRT - <s>					Echo lines from stdin containing <s> + final count
//  FRT -v - <s>				Echo lines from stdin NOT containing <s> + final count
//  FRT <wc> <s>				Find files <wc>, echo lines containing <s> [+ count] (grep)
//  FRT -v <wc> <s>			Find files <wc>, echo lines NOT containing <s> [+ count]

//  FRT - <s> <r>				Echo lines from stdin containing <s>, but show <r> + final count
//  FRT -v - <s> <r>			Echo lines from stdin containing <s>, but show <r> + final count
//  FRT <wc> <s> <r>			Find files <wc> with <s>, replace with <r>, show filenames + count
//  FRT -v <wc> <s> <r>		?
//  FRT -p <wc> <s> <r>		Find files <wc> with <s>, show filenames, print lines with <r> + count
//  FRT <wc> <s> "r"			Find files <wc>, remove lines with <s>, show filenames + count
//  FRT -v <wc> <s> "r"		Find files <wc>, remove lines NOT with <s>, show filenames + count

// TODO:
// * don't touch files if nothing changed							done
// * prevent processing a file twice when frt'ing filenames		done
...
static const char __temp_file[16] = "_frt.~";
static const char __backup_suffix[16] = ".bak";			// frt.cpp.bak
...
void usage()
{
	...
	printf("Usage: frt [options] [--] <wildcard>[%c...] [find_string] [replace_string]\n", _WILDCARD_SEPARATOR );
	printf("\nOptions:\n");
...
int _findtext( FILE* f, const char *filename )
{
	...
	{
		ln++;
		if (!fgets( frt_buf, MAXSTRING, f ))
			break;

//		char *t = findtext_line( frt_buf );
		// no need to know the exact occurence, just count
		int t = findtext_line_count( frt_buf );
...
const char* pre_frt( const char* test )
{
...
int frt_line( const char *_line, char *frted )
{
	...
	int count = 0;
	char *output = frted;			// output pointer
	size_t offset, cur = 0;
	...
	for (const char *t;(t = strstr( compare_buf+cur, FindString ));cur=offset+FindLength)
	{
		...
		const char* replacement = pre_frt(_line+offset);
		if (!replacement)
			continue;
...
static char fout[256] = {0};
	...
	size_t tot = fread( frt_buf, 1, MAXSTRING, f );
	size_t noascii=0;
	for (size_t b=0;b<tot;b++)
	{
		unsigned char t = frt_buf[b];
...
bool findtext_file( const char* in )
{
	...
	{
		if (_Verbose)
			ERRPRINTF1( "FRT: skipping binary file: %s\n", in );
		fclose(f);
		return false;
	}
...
int _frt( FILE *f1, FILE *f2, const char* in )
{
	int	this_find_count=0;					// Number of occurences in this file
	bool first = true;
	bool replace = false;
	...
	{
		ln++;
		if (!fgets( frt_buf, MAXSTRING, f1 ))
			break;

		const char* b = get_compare_buf(frt_buf);
		...
		while (1)
		{
			...
			if (!t)
			{
				...
				if (f2)
					fputs( frt_buf+(bp-b), f2 );
				break;
			}

			// Adapt the replace_string to the actually found string
			const char *replacement = pre_frt( frt_buf+(t-b) );
			if (!replacement)
			{
				if (f2)
					fputs( frt_buf+(bp-b), f2 );
...
bool frt( const char* in )
{
	...
	int this_find_count = _frt(f1,f2,in);
...
// FIXME: when frt'ing filenames, we might process a file twice!
...
int for_all_files_recursive( const char *dir, const char* wc, file_func_t _ff )
{
	...
	for (int t=0;spul[t];free(spul[t]),t++)
	{
		...
		{
			if (_Verbose)
				ERRPRINTF2( "FRT: skipping cvs dir %s%s\n",dir, spul[t] );
			continue;
		}
		...
		{
			if (_Verbose)
				ERRPRINTF2( "FRT: skipping svn dir %s%s\n",dir, spul[t] );
			continue;
		}
		...
		{
			if (_Verbose)
				ERRPRINTF2( "FRT: skipping git dir %s%s\n",dir, spul[t] );
			continue;
		}
...
void parse_options( int argc, char* argv[] )
{
	...
	for (int t=1;t<argc;t++)
	{
		...
		{
			if (_Verbose)
				ERRPRINTF1( "FRT: replace_string=\"%s\"\n", argv[t] );
...
int frt_file_path( const char* dir, const char* file )
{
	char *_path = strdup2(dir,file);

	if (_Names)
	{
		int count = frt_line(file,frt_buf);
		if (count)
		{
			// frt_buf contains the new filename
			TotalFileCount++;

			char *newpath = strdup2(dir,frt_buf);
			...
			if (_Preview || rename( _path, newpath )==0)
			{
				...
				if (!_Preview)
					TotalFindCount += count;
				printf( __filename_rename, _path, frt_buf);
			}
			else
			{
...
int main( int argc, char* argv[] )
{
	...
	if (_CStyle && FindLength)
	{
		...
		if (_Verbose)
			ERRPRINTF1( "FRT: actual find_length=%i\n", FindLength );
	}
	if (_CStyle && ReplaceLength)
	{
		ReplaceLength = cstyle(ReplaceString);
		if (_Verbose)
			ERRPRINTF1( "FRT: actual replace_length=%i\n", ReplaceLength );
	}
	...
	bool grepMode = (ReplaceLength==0);						// grep or frt?

	// OPTIMIZE: Check for redundant FRT (where find_string==replace_string)
...
//			strlwr( ReplaceStringLwr );							// FIXME: memlwr
		...
		else
		{
			...
			if (i && _Verbose)
				ERRPRINTF1( "FRT: actual replace_string=\"%s\"\n", ReplaceString );
			_AdaptCase = false;
		}
	}
	...
	if (strcmp( WildCard, "-" )==0)
	{
		// FRT in stdin/stdout
		// If asked to check filenames, just return 0 (pointless)
		int count = _Names?0:_frt( stdin, stdout, NULL );
		if (!_Quiet)
	...
	if (_Binary && !_Preview)
	{
		...
		// Warn about FRT'ing binary files
		ERRPRINTF( "Warning: frt may corrupt binary files\n" );
	}

	for_all_wildcards( WildCard, &frt_file_path );
	if (!_Quiet)
